...
describe('calendar home groups',()=>{
 ...
 it('chooses the nearest deadline even when the API input is not ordered',()=>{const later=event({id:'later',start:'2026-09-12',category:'Deadline'});const sooner=event({id:'sooner',start:'2026-09-10',category:'Deadline'});expect(groupCalendarEvents([later,sooner],new Date('2026-09-08T00:00:00Z')).nextDeadline?.id).toBe('sooner')});
 it('does not treat a meeting as a deadline when no deadline exists',()=>{const meeting=event({id:'meeting',start:'2026-09-09T10:00:00+09:00',category:'Meeting'});expect(groupCalendarEvents([meeting],new Date('2026-09-08T00:00:00Z')).nextDeadline).toBeNull()});
});
