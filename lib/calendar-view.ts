...
export function groupCalendarEvents(events:CalendarEvent[],now=new Date()){
 const todayKey=seoulDateKey(now);const today=events.filter(event=>eventDateKey(event)===todayKey);const upcoming=events.filter(event=>eventDateKey(event)>todayKey);const nextDeadline=[...upcoming].filter(event=>event.category==='Deadline').sort((a,b)=>compareText(eventDateKey(a),eventDateKey(b))||compareText(a.start,b.start)||compareText(a.id,b.id))[0]||null;
